...
// Synapses-----------------------------------------------
inline float spike_velocity = 1.0;
...
struct neuro_node_t
{
	...
	virtual void output([[maybe_unused]] event_t &&e) {} //  worker.events.enqueue(std::move(ev));
	...
	neuro_node_t(const neuro_node_t &&other) : u_mem(other.u_mem),
											   threshold(other.threshold),
											   last_fired(other.last_fired) {}
};
...
struct layer_t
{
	...
	virtual void worker(const event_t &&e) = 0;
...
struct cortex_layer_t : layer_t
{
	...
	std::vector<cortex_node_t> neurons_storage;
	...
	void worker(const event_t &&e) override final;
...
struct retina_layer_t : layer_t
{
	...
	std::vector<retina_node_t> neurons_storage;
...
struct mnist_couch_layer_t : layer_t
{
	...
	std::vector<couch_node_t> neurons_storage;
	unsigned char label;
	node_t &node_ref(layer_dim_t row, layer_dim_t col) override final
	{
		return neurons_storage.at(neurons.at(row).at(col));
	};
	void worker(const event_t &&e);
...
struct sheperd_t
{
	ThreadSafeQueue sensor_queue;
	ThreadSafeQueue co_sheph_queue;
	ThreadSafeQueue local_queue;
	std::jthread tr;
	void shepherd()
	{
	}
